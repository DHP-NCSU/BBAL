...
def ceal_learning_algorithm(du: DataLoader,
    ...
    model = AlexNet(n_classes=256, device=device).to(device)
    ...
    for iteration in range(max_iter):
        ...
        logger.info('Iteration: {}: run prediction on unlabeled data `du` '.format(iteration))

        # Run predictions on `du` and move data to GPU
        ...
        logger.info('Update size of `dl`  and `du` by adding uncertain {} samples in `dl`'
                    ' len(dl): {}, len(du) {}'.format(len(uncert_samp_idx), len(dl.sampler.indices), len(du.sampler.indices)))
        ...
        logger.info('Update size of `dl`  and `du` by adding {} hcs samples in `dl`'
                    ' len(dl): {}, len(du) {}'.format(len(hcs_idx), len(dl.sampler.indices), len(du.sampler.indices)))

        if iteration % t == 0:
            logger.info('Iteration: {} fine-tune the model on dh U dl'.format(iteration))
        ...
        logger.info('remove {} uncertain samples from du'.format(len(uncert_samp_idx)))
...
if __name__ == "__main__":
    ...
    dataset_train = Caltech256Dataset(
        ...
        transform=transforms.Compose(
            [SquarifyImage(), RandomCrop(224), Normalize(), ToTensor()]))
    ...
    # Run the learning algorithm with GPU support
